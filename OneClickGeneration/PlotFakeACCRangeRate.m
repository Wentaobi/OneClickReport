...
title('ACC Range Rate performance');
xlabel('Time [Unit: 0.01s/step]');
ylabel('Range Rate, Speed[Host-Target], Unit: m/s')
grid on;
...
de = 0.5;
ne = 50;
plot([0, L], [ne, ne],'-r','LineWidth',2);
xlim auto
ylim([sigma-2, sigma+2])
plot([0, L], [ne+de, ne+de],'--g', [0, L], [ne-de, ne-de],'--g','LineWidth',2);
legend('ACC Actual Range Rate','ACC Default Range Rate','ACC Stable Range Rate');
hold on;
...
%%
text(0,ne+3*de,'Confidence Level: 95%','Color','blue','FontSize',12)
